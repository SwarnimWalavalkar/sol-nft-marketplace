...
describe("Marketplace", function () {
  ...
  async function mintTokenAndCreateMarketItem(
    tokenId: number,
    price: any,
    transactionOptions: { value: any },
    account = owner
  ) {
    await nftContract.connect(account).mintToken("");
    return marketplaceContract
      .connect(account)
      .createMarketItem(nftContractAddress, tokenId, price, transactionOptions);
  }
  ...
  it("creates a Market Item", async function () {
    ...
    // Act and Assert
    await expect(
      mintTokenAndCreateMarketItem(tokenId, price, transactionOptions)
    )
  ...
  it("creates successive market items for the same token id by buying and selling", async function () {
    ...
    await marketplaceContract
      .connect(buyer)
      .createMarketSale(nftContractAddress, token1marketItemId, {
        value: price,
      });
    ...
    await marketplaceContract
      .connect(buyer)
      .createMarketItem(nftContractAddress, token1id, price, {
        value: listingFee,
      });
  ...
  it("reverts when trying to cancel an inexistent market item", async () => {
    // Act and Assert
    expect(
      marketplaceContract.cancelMarketItem(nftContractAddress, 1)
  ...
  it("reverts when trying to cancel a market item whose seller is not msg.sender", async () => {
    ...
    // Act and Assert
    expect(
      marketplaceContract.connect(buyer).cancelMarketItem(nftContractAddress, 1)
  ...
  it("reverts a Market Sale if offer price is not the same as listing price", async function () {
    ...
    expect(
      marketplaceContract
        .connect(buyer)
        .createMarketSale(nftContractAddress, 1, { value: offerPrice })
